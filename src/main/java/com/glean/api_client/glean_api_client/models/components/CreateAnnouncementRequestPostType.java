/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 * @generated-id: 7ade355c7cb8
 */
package com.glean.api_client.glean_api_client.models.components;

import com.fasterxml.jackson.annotation.JsonValue;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;

/**
 * CreateAnnouncementRequestPostType
 * 
 * <p>This determines whether this is an external-link post or a regular announcement post. TEXT - Regular
 * announcement that can contain rich text. LINK - Announcement that is linked to an external site.
 */
public enum CreateAnnouncementRequestPostType {
    TEXT("TEXT"),
    LINK("LINK");

    @JsonValue
    private final String value;

    CreateAnnouncementRequestPostType(String value) {
        this.value = value;
    }
    
    public String value() {
        return value;
    }
    
    public static Optional<CreateAnnouncementRequestPostType> fromValue(String value) {
        for (CreateAnnouncementRequestPostType o: CreateAnnouncementRequestPostType.values()) {
            if (Objects.deepEquals(o.value, value)) {
                return Optional.of(o);
            }
        }
        return Optional.empty();
    }
}

