# UpdateAnnouncementRequestPostType

This determines whether this is an external-link post or a regular announcement post. TEXT - Regular announcement that can contain rich text. LINK - Announcement that is linked to an external site.

## Example Usage

```java
import com.glean.api_client.glean_api_client.models.components.UpdateAnnouncementRequestPostType;

UpdateAnnouncementRequestPostType value = UpdateAnnouncementRequestPostType.TEXT;
```


## Values

| Name   | Value  |
| ------ | ------ |
| `TEXT` | TEXT   |
| `LINK` | LINK   |